...
gf框架的模板引擎建立在标准库模板引擎之上，本章先介绍gf框架的模板引擎使用，随后介绍标准库模板引擎的基本语法。

# 使用方法

## 在控制器中

gf为控制器提供了良好的模板引擎支持，由gmvc.View视图对象进行管理，由于使用控制器注册方式处理请求时，每个控制器是独立的，因此控制器中的视图对象也是独立的。控制器视图是并发安全设计的，允许在多线程中异步操作。
