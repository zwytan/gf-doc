...
在Web开发中，模板引擎是最核心的组件之一，是连接控制器与视图层的桥梁。Go标准库的text/template包已经提供了非常强大的模板引擎功能，gf框架的模板引擎以此为基础，并在易用性、功能性、数据隔离性方面做了一定封装和改进，提供了gview包。此外，在内置的MVC开发模式中，每一个控制器注册的对象中都会有一个gmvc.View成员对象，用于不同请求的模板渲染处理。

>[danger] # 使用方法

>[success] ## 控制器视图

gf为控制器提供了良好的模板引擎支持，由gmvc.View视图对象进行管理。控制器视图是并发安全设计的，允许在多线程中异步操作。
...
非控制器中使用模板引擎没有控制器视图的支持，可以使用底层的gview包来实现，并通过单例管理器来获取默认的单例gview对象。
gview包方法列表：
