...
我们可以通过```BindControllerMethod```方法完成对控制器指定方法的注册。相对于```BindController```注册方法，```BindControllerMethod```仅仅多了一个方法名称参数```methods```(也可以由```BindController```的第三个非必需参数替换)，参数支持传入多个方法名称，多个名称以英文```,```号分隔（**方法名称参数区分大小写**）。

看下面这个例子，执行后```ControllerMethod```的```Name```和```Age```方法将被注册到Web Server提供服务，而```Info```方法却不会对外公开。
