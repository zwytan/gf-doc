...
ghttpController接口中的Init和Shut是两个在HTTP请求流程中被Web Server自动调用的方法。gmvc.Controller已经实现了这两个方法，用户自定义的控制器类直接继承gmvc.Controller即可。如果需要自定义请求初始化一级请求结束时的一些业务逻辑操作，可以在自定义控制器中覆盖这两个方法。
