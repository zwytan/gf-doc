...
> 建议性的约定前缀：这个属于开发技巧建议。对于项目结构中的 控制器层`/app/api`下的包名统一使用`a_`前缀；逻辑封装层`/app/service`下的包名统一使用`s_`前缀；数据模型`/app/model`下的包名统一使用`m_`前缀。这样将更有助于相同包名在代码中的区分、阅读和维护，此外对于IDE也很友好，例如在IDE中输入前缀(如:`s_`)后会自动代码提示过滤，迅速定位到所需的包。
