用户可以通过路由控制来自定义URI与注册服务的关联关系，gf的路由控制规则依靠正则表达式来实现，相对来说比较专业，灵活度比较高，但是另一方面，针对于对正则表达式不熟悉的同学，配置起来可能不是那么方便。路由控制是由Web Server自动调用的(Dispatch/Patch)，用户只需调用路由规则注册/删除方法(Set*/Remove*)完成注册即可.
