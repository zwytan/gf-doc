...
访问 http://127.0.0.1:8199/upload/show 选择需要上传的文件，此外还有一个测试的输入域，用以服务端测试同时获取文件和提交参数。提交之后可以看到文件上传成功到服务器上，并且显示出输入域提交的值。

文件上传比较简单，但是需要注意的是，服务端在上传处理中需要使用defer f.Close() 关闭掉临时上传文件指针。
