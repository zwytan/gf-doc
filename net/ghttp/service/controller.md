...
在控制器注册中，控制器对象还有一个```Exit```方法，供业务层直接调用，该方法的作用是**使当前业务执行流程立即退出，不再执行后续的服务处理（但是不影响事件回调处理）**。假如在```Init```回调方法中调用```Exit```，那么后续的服务逻辑将不会执行。假如在```Shut```方法中调用```Exit```，如果事件回调方法不做任何处理的话，那么该操作其实没有任何意义。
