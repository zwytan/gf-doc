...
此外，我们可以通过```ghttp.Server.SetNameToUriType```方法来设置struct/method名称与uri的转换方式。支持的方式目前有3种，对应3个常量定义：
```go
...
func main() {
    ...
    s1.BindObject("/user", new(User))
    s2.BindObject("/user", new(User))
    s3.BindObject("/user", new(User))
