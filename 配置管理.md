...
为简化Web Server的运行配置，ghttp默认配置目录为当前执行文件目录，并将配置路径下的config.json配置文件作为默认全局配置文件，并可在运行时通过SetPath修改配置目录。config.json全局配置文件中默认集成了gf框架的几个核心组件配置项，包括：视图管理、路由控制及数据库管理（当然用户可以在程序中自定义修改核心组件的初始化以及对应配置项，具体查看单例管理部分）。
