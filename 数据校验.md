...
```
可以看到，我们可以对在struct定义时使用```gvalid```的标签属性来绑定校验的规则及错误提示信息。在标签中，校验规则与错误提示以```#```符号进行分隔，当然，错误提示信息并不是必须的。

在使用```CheckStruct```方法对struct对象进行校验时，也可以传递校验或者和错误提示参数，这个时候会覆盖struct在定义时绑定的对应参数。
