...
在以上示例中，```Age```属性由于默认值0的存在，因此会引起required规则的失效，因此这里没有使用```required```规则而是使用```between```规则来进行校验。
...
```
可以看到，我们可以对在struct定义时使用```gvalid```的标签属性来绑定校验的规则及错误提示信息。标签规则如下：
```
[返回名称@]校验规则[#错误提示]
```
其中，返回名称和错误提示为非必需字段。**返回名称**指定校验成功后的map中的属性名称，例如在处理请求表单时比较有用，因为表单的字段名称往往和struct的属性名称不一致；**校验规则**是必需参数，具体请查看本章节上面的相关说明；**错误提示**表示自定义的错误提示信息，当规则校验时对默认的错误提示信息进行覆盖。
