...
gcfg包最大的特点是支持按层级获取配置数据，层级访问通过英文"```.```"号指定，其中```pattern```参数和[gjson包](encoding/gjson/index.md)的pattern参数一致。参数files指定需要读取的配置文件目录下的文件名称，是一个可选参数，默认为```config.yml```（即读取config.yml配置文件数据）。yml类型文件也是推荐的配置文件格式。
