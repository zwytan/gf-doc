...
其中HttpMethod和域名为非必需参数，一般来说直接给定路由规则（路由规则分为静态路由和动态路由，为便于演示本章节所有服务注册均采用静态路由规则，路由规则的详细介绍请查看[【路由控制】](路由控制.md)章节）参数即可。因为需要使用HttpMethod注册的情况大多数为RESTful控制器，直接使用RESTful相关方法注册即可，域名支持也可以使用Domain方法来进行绑定。
